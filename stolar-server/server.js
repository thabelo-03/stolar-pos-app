...
mongoose.connect(dbURI, {
  ...
  .then(() => {
    console.log("✅ MongoDB Connected: Stolara POS Cluster");
  })
...
// 1. GET CURRENT RATES FOR A SHOP
app.get('/api/shops/rates/:shopId', async (req, res) => {
  try {
    const shop = await Shop.findById(req.params.shopId);
    if (!shop) return res.status(404).json({ message: "Shop not found" });
    res.json({ rates: shop.rates || { ZAR: 19.2, ZiG: 26.5 } });
  } catch (err) {
...
app.post('/api/sales', async (req, res) => {
  try {
    ...
    const newSale = new Sale({ items, totalUSD, totalPaidLocal, currencyUsed, rateUsed, paymentMethod, date, offlineId });
    await newSale.save();
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 Stolara PPA Server running on port ${PORT}`);
});